...
RUN uv pip install --system \
    ...
    azul-plugin-index-coincidence==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    uv pip install --system --find-links /tmp/ azul-plugin-index-coincidence==$(cd /tmp/src && python -m setuptools_scm);fi
